...
const tests = {
	testFlow: async () => {
		...
		assertEquals(
			"User B's Post",
			$old("main-content-wrapper favorites favorite[post] post h2").textContent.trim(),
		...
		assertEquals(
			...
			$old("main-content-wrapper favorites favorite[post] p span").textContent,
			"Should have a post favorite",
		)
		assertEquals(
			"Great point!",
			$old("main-content-wrapper favorites favorite[reply] p span").textContent,
			"Should have a reply favorite",
		)

		// Test navigation flow: favorites -> reply -> post -> back to reply -> back to favorites
		$old("main-content-wrapper favorites favorite[reply]").click()
		await new Promise(resolve => setTimeout(resolve, 0))
		...
		$old("tab-wrapper tab-item[right]").click()
		await new Promise(resolve => setTimeout(resolve, 0))
		...
		$old("tab-wrapper tab-item:not([right])").click()
		await new Promise(resolve => setTimeout(resolve, 0))
