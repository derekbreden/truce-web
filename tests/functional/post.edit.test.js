...
const tests = {
	testFlow: async () => {
		...
		$old("main-content post icon[more]").click()
		await new Promise(resolve => setTimeout(resolve, 0))
		...
		assertEquals(
			...
			$old("add-new[post] textarea[body]").value,
			"Edit form should be pre-filled with existing body",
		)
		
		// Modify only the body (keep same title to avoid slug change and redirect)
		$old("add-new[post] textarea[body]").value = "This is User A's edited post content with much more text than the original to pass validation."
		...
		assertEquals(
			"User A's Post",
			$old("main-content post h2").textContent.trim(),
		...
		assertEquals(
			...
			$old("main-content post p span").textContent,
