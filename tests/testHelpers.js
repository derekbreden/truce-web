...
function loadAllClientScripts(customIndexPath) { // Added customIndexPath parameter
  ...
  try {
    ...
    const scriptRegex = /\/\/\s*<!--#include\s+file="([^"]+\.js)"\s*-->/g;
    let match;
    const scriptFiles = [];
    while ((match = scriptRegex.exec(indexHtmlContent)) !== null) {
      ...
      scriptFiles.push(path.resolve(indexDir, match[1]));
    }

    if (scriptFiles.length === 0) {
      console.warn("No script files found in index.html based on the pattern.");
