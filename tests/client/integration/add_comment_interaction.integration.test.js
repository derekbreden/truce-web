...
const tests = {
	testAddCommentInteraction: async () => {
		...
		// 9. Verify the fetch was called with correct data
		assertEquals(
			true,
		...
		// 10. Verify form shows "Validating..." message after submission
		const $validatingMessage = $("add-new[comment] info")
		assertEquals(
			"Validating...",
			$validatingMessage.innerText.trim(),
			"Should show 'Validating...' message.",
		)

		// 11. Verify form controls have disabled attribute during submission
		const $disabledTextarea = $("add-new[comment] textarea[body][disabled]")
		const $disabledSubmitButton = $("add-new[comment] button[submit][disabled]")
		
		assertEquals(
			"",
			$disabledTextarea.getAttribute("disabled"),
			"Textarea should have disabled attribute during submission.",
		)
		assertEquals(
			"",
			$disabledSubmitButton.getAttribute("disabled"),
