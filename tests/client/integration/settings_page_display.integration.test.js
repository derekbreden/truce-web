    ...
    testSettingsPageDisplaysCorrectly: async () => {
        ...
        window.setMockFetchResponseForPaths({
            ...
            "/topics": { // For navigation after agreeing to terms
                ...
                profile_picture_uuid: null, display_name_index: 0,
                subscribed_to_users: 0,
                has_more: false
            },
            "/settings": { // For the actual settings page
                path: "/settings",
        ...
        assertEquals(true, Boolean($settingsLink), "Settings link should exist in the menu.")
        $settingsLink.click()
        await new Promise(resolve => setTimeout(resolve, 0)) // Wait for settings page to render
        assertEquals("/settings", state.path, "Path should be /settings after clicking the settings link.")

        // 4. Verify Settings Page Content
        const $mainContentWrapper = $("main-content-wrapper[active]")
        assertEquals(true, Boolean($mainContentWrapper), "Main content wrapper for settings page should be active.")
        ...
        const $settingsPageHeaderSpan = $mainContent.$("topics topic h2[settings] > span:first-child")
        assertEquals(true, Boolean($settingsPageHeaderSpan), "Settings page H2 header span should exist (topics > topic > h2[settings] > span:first-child).")
        assertEquals("Account settings", $settingsPageHeaderSpan.innerText.trim(), "Settings page H2 header text should be 'Account settings'.")
        ...
        assertEquals(true, Boolean($displayNameInput), "Display name input field (input[display-name]) should exist on the settings page.")
        assertEquals("Test User", $displayNameInput.value, "Display name input should be pre-filled with the user's current display name.")
        ...
        assertEquals(true, Boolean($removeAccountButton), "Remove Account button should exist on the settings page.")
        assertEquals("Remove Account", $removeAccountButton.innerText.trim(), "Remove Account button text should be 'Remove Account'.")

        const $saveDisplayNameButton = $mainContent.$("button[save]")
        assertEquals(true, Boolean($saveDisplayNameButton), "Save Display Name button should exist on the settings page.")
        assertEquals("Save display name", $saveDisplayNameButton.innerText.trim(), "Save Display Name button text should be 'Save display name'.")
    }
}
