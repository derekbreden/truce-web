...
const tests = {
	...
	testCreateUserOnFavoriteActions: async () => {
		...
		for (let i = 0; i < favorite_actions.length; i++) {
			const action = favorite_actions[i]
			const req = createMockRequest(
				action,
				{ session_id: "123", user_id: undefined }
			)
	...
	testCreateUserOnBlockActions: async () => {
		...
		for (let i = 0; i < block_actions.length; i++) {
			const action = block_actions[i]
			const req = createMockRequest(
				action,
				{ session_id: "123", user_id: null }
			)
	...
	testCreateUserOnFlagActions: async () => {
		...
		for (let i = 0; i < flag_actions.length; i++) {
			const action = flag_actions[i]
			const req = createMockRequest(
				action,
				{ session_id: "123", user_id: undefined }
			)
	...
	testNoActionWhenUserExists: async () => {
		...
		const original_user_id = req.session.user_id
	...
	testPollChoiceRequiresBothFields: async () => {
		...
		for (let i = 0; i < incomplete_polls.length; i++) {
			const poll_data = incomplete_polls[i]
			const req = createMockRequest(
				poll_data,
				{ session_id: "123", user_id: undefined }
			)
			...
			assertEquals(
				undefined,
				req.session.user_id,
				`Should not create user for incomplete poll data: ${JSON.stringify(poll_data)}.`
			)
		}
	}
}
