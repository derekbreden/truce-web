...
let bcryptCompareCalls = []
let bcryptHashCalls = []
const mockBcrypt = {
	compare: (plaintext, hash) => {
		bcryptCompareCalls.push({ plaintext, hash })
	...
	hash: (plaintext, saltRounds) => {
		bcryptHashCalls.push({ plaintext, saltRounds })
		// Return a mock hash
...
const tests = {
	testSuccessfulSignIn: async () => {
		...
		assertEquals(
			1,
			bcryptCompareCalls.length,
			"Should call bcrypt.compare once."
		)
		assertEquals(
			'correctPassword',
			bcryptCompareCalls[0].plaintext,
			"Should compare with correct password."
		)
		assertEquals(
			'stored_hash_value',
			bcryptCompareCalls[0].hash,
	...
	testSignUpNewUser: async () => {
		...
		assertEquals(
			1,
			bcryptHashCalls.length,
			"Should call bcrypt.hash once."
		)
		assertEquals(
			'newPassword',
			bcryptHashCalls[0].plaintext,
			"Should hash the provided password."
		)
		assertEquals(
			12,
			bcryptHashCalls[0].saltRounds,
