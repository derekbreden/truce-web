...
CREATE INDEX idx_posts_create_date_counts_max_create_date 
ON posts(create_date, counts_max_create_date)
STORING (reply_count, favorite_count)
CREATE INDEX idx_posts_counts_max_create_date ON posts(counts_max_create_date) STORING (reply_count, create_date, favorite_count)
...
CREATE INDEX idx_replies_create_date ON replies(create_date) STORING (parent_post_id, parent_reply_id, body, note, user_id, favorite_count, counts_max_create_date)
CREATE INDEX idx_replies_parent_reply_id ON replies(parent_reply_id)
CREATE INDEX idx_replies_parent_post_id ON replies(parent_post_id) STORING (user_id)
CREATE INDEX idx_replies_create_date_counts_max_create_date 
ON replies(create_date, counts_max_create_date)
STORING (favorite_count)
CREATE INDEX idx_replies_parent_post_id_parent_reply_id_user_id ON replies (parent_post_id, parent_reply_id, user_id) STORING (body, note, create_date, favorite_count, counts_max_create_date)

CREATE TABLE IF NOT EXISTS reply_ancestors (
reply_id INT NOT NULL,
ancestor_id INT NOT NULL
)
CREATE INDEX idx_reply_ancestor ON reply_ancestors (reply_id, ancestor_id)
ALTER TABLE reply_ancestors
ADD CONSTRAINT unique_reply_ancestor UNIQUE (reply_id, ancestor_id)
...
CREATE INDEX idx_favorites_posts_user_id ON favorite_posts(user_id) STORING (post_id)
CREATE INDEX idx_favorites_posts_post_id ON favorite_posts(post_id) STORING (user_id)
...
CREATE INDEX idx_favorites_replies_user_id ON favorite_replies(user_id) STORING (reply_id)
CREATE INDEX idx_favorites_replies_reply_id ON favorite_replies(reply_id) STORING (user_id)
...
CREATE INDEX idx_poll_votes_post_id_user_id ON poll_votes(post_id, user_id)

CREATE TABLE IF NOT EXISTS post_topics (
post_id INT NOT NULL,
topic_id INT NOT NULL
)
CREATE INDEX idx_post_topics_post_id_topic_id ON post_topics(post_id, topic_id)
...
INSERT INTO TAGS (topic_name, subtitle) VALUES
