	...
	Array.from($old("[profile-picture] input[image]").files).forEach((file) => {
		...
		reader.onload = ($event) => {
			imageToPng(
				$event.target.result,
				(png) => {
					const $imagePreviewContainerById = window.document.getElementById(
						"testImagePreviewArea",
					)
					const $original = $imagePreviewContainerById
					...
					if ($imagePreviewContainerById) {
						// Guard actual replacement
