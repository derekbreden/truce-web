...
3. Understanding innerText vs. textContent in the Test Environment:
		...
		// assertEquals("Title", $header.innerText.trim(), "...")

		// Correct for Flint.js text set in a child span:
		const $headerSpan = $("h2[page-title] span")
...
2.	**Assertion Style:**
		*	 **Strictly Avoid Guard Assertions - Prefer Direct Failures:** Do not use "guard assertions" such as `assertEquals(true, Boolean(element), "Element should exist")` before attempting to interact with an element. If an element is expected to be present, tests should interact with it directly by attempting to access its properties or call its methods (e.g., `const text = $element.$("child").innerText` or `$element.click()`). If `$element` or `$("child")` is unexpectedly `null` or `undefined`, the test will fail appropriately at the point of access (e.g., with an error like "Cannot read properties of null (reading 'innerText')"). This type of direct failure is preferred because it provides a more precise stack trace and clearly indicates which expected element was missing or which interaction failed, without the noise of a preceding, less informative guard assertion. Guard assertions are redundant, obscure the primary test objective, and offer less useful failure information than a direct language-level error.
