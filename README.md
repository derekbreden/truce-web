...
2.	**Assertion Style:**
		*	 **Strictly Avoid Guard Assertions - Prefer Direct Failures:** Do not use "guard assertions" such as `assertEquals(true, Boolean(element), "Element should exist")` before accessing an element's properties or methods. Instead, directly attempt to access the property or call the method (e.g., `const text = $element.$("child").innerText` or `$element.click()`). If `$element` or `$("child")` is unexpectedly null or undefined, the test will fail at the point of access (e.g., "Cannot read properties of null (reading 'innerText')"). This type of direct failure provides a more precise stack trace and clearly indicates the exact expectation that failed, without the noise of a preceding, less informative guard assertion. Guard assertions are redundant, obscure the primary test objective, and offer less useful failure information than a direct crash.
